...
static int ping_pong_client() {
    ...
    auto cli = photon::net::new_fstack_dpdk_socket_client();
...
static int echo_server() {
    ...
    if (FLAGS_vcpu_num > 1) {
        work_pool = new photon::WorkPool(FLAGS_vcpu_num, photon::INIT_EVENT_DEFAULT, photon::INIT_IO_FSTACK_DPDK);
    }
...
int net_perf_echo_server() {
    return echo_server();
}

int net_perf_ping_pong_client() {
    return ping_pong_client();
}
